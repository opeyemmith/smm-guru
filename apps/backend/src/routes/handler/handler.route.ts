...
import type { HonoAuthSession } from "../../lib/better-auth/types.auth.js";
import type { HandlerTypes } from "../../type/types.handler.js";
...
const handlerRoute = new Hono<HonoAuthSession>();

handlerRoute.post("/", async (c) => {
  const userId = c.get("user-id")!;
  const body = (await c.req.json()) as HandlerTypes;
  ...
  if (body.action === "add") {
    const serviceDetails = await db.query.servicesSchema.findFirst({
      where: and(
        ...
        eq(servicesSchema.userId, userId)
      ),
      columns: {
        providerId: true,
      },
    ...
    const providerDetails = await db.query.providersSchema.findFirst({
      where: and(
        ...
        eq(providersSchema.userId, userId)
      ),
      columns: {
        id: true,
        apiKey: true,
